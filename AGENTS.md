...
**`ci.yml` notes:** CI runs lint, typecheck, unit tests, `dbt build` (models + data/unit tests), Go tests, integration-db, Docker build, and Terraform validate on PR/push to `develop`, `staging`, and `main`.
