...
# Simple cross-process file lock to serialize asset execution across processes
@contextmanager
def file_lock(lock_path: str = "/app/src/dagster/locks/assets.lock"):
	os.makedirs(os.path.dirname(lock_path), exist_ok=True)
	# Open a file descriptor and acquire an exclusive lock (blocks until available)
	with open(lock_path, "w+") as fh:
		try:
			fcntl.flock(fh.fileno(), fcntl.LOCK_EX)
		except IOError as e:
			# If locking is not supported or fails, raise so caller can decide
			raise
		try:
			yield
		finally:
