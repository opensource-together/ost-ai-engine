...
from src.linker.utils.serialization import make_serializable
...
def core_github__detect_languages(
    ...
    for i, repo in enumerate(projects):
        ...
        if text and has_non_latin_chars(text):
    ...
    sample = make_serializable(clean_sample)
    filtered = make_serializable(filtered_out_projects)
    meta = {
