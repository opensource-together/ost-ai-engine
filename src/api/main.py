...
class RecommendationResponse(BaseModel):
    ...
    user_id: int = Field(..., description="The user ID for whom recommendations were generated")
    recommended_projects: list[UUID] = Field(..., description="List of recommended project IDs")
...
app = FastAPI(
    ...
    description="API for the recommendation engine that suggests relevant projects to users based on their interests and activities.",
    version="0.1.0",
    lifespan=lifespan,
)
...
@app.get("/recommendations/{user_id}",
         ...
         responses={
             404: {"model": ErrorResponse, "description": "User not found or no interest profile"},
             500: {"model": ErrorResponse, "description": "Model not available or internal error"}
         },
         summary="Get Project Recommendations",
         description="Get personalized project recommendations for a user based on their contributions, applications, and team memberships")
async def get_recommendations(
    ...
    if (app.state.similarity_matrix is None or 
        app.state.projects is None or 
        len(app.state.projects) == 0):
        raise HTTPException(
            status_code=500,
            detail="Recommendation models are not available. Please run the training pipeline first."
        )
    ...
    try:
        ...
        if not interested_project_ids:
            raise HTTPException(
                status_code=404,
                detail=f"No interest profile found for user {user_id}. User needs to interact with projects first."
            )
