"""Unit tests for Go fetcher readme asset wiring."""
...
from src.linker.assets.scraper.core_github__fetch_readme import core_github__fetch_readme
from src.linker.resources.cfg_resource import PipelineConfig


class TestCoreGithubFetchReadme:
    def test_runtime_error_when_fetcher_binary_missing(self) -> None:
        ...
        with patch("os.path.exists", return_value=False):
            with pytest.raises(RuntimeError, match="Go binary not found"):
                core_github__fetch_readme(context=context, core_github__detect_languages=df)

    @patch("src.linker.assets.scraper.core_github__fetch_readme.subprocess.run")
    def test_invokes_fetcher_with_readme_mode(
        self,
        mock_run: MagicMock,
    ) -> None:
        ...
        with patch("os.path.exists", return_value=True):
            output = core_github__fetch_readme(
                context=context,
        ...
        assert cmd == ["/bin/ost-fetcher", "--mode", "readme", "--concurrency", "20"]
